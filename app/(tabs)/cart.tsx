...
// export default function CartScreen() {
//   const { cart, wishlist, removeFromCart, updateCartQuantity } = useCart();
//   const router = useRouter();
...
//     return cart
//       .reduce((sum, item) => {
//         // Ensure the price is valid and can be parsed
...
//       {/* Wishlist Section */}
//       <VStack space="lg" className="p-5 bg-white rounded-lg shadow-md mt-5">
//         <Text className="text-2xl font-bold mb-4 text-typography-900">
//           Your Wishlist
//         </Text>
//         {wishlist.length > 0 ? (
...
export default function CartScreen() {
  ...
  const calculateTotal = () => {
    return cart
      .reduce((sum, item) => {
        // Ensure selling_price is valid and can be parsed
        const price = item.selling_price
          ? parseFloat(item.selling_price.toString().replace(/,/g, ""))
          : 0;
        return sum + item.quantity * price;
      }, 0)
      .toFixed(2);
